...
def semisupervised(vol_names, seg_names,batch_size,atlas_file=None, downsize=2):
    """
    ...
    gen = volgen(vol_names, segs=seg_names, batch_size = batch_size,np_var='vol')
    zeros = None
...
def seg_volgen(
    ...
    while True:
        ...
        for i in range(len(vol_names)):
            imgs = [py.utils.load_volfile(vol_names[i], **load_params)]
            # print(vol_names[i])
            vols = [np.concatenate(imgs, axis=0)]
            yield tuple(vols)

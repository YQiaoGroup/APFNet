...
class wavelet_transform(nn.Module):
    ...
    def forward(self, input_volume):
        ...
        coeffs_low, coeffs_high = ptwt.wavedec3(input_volume.squeeze(0), "haar", level=self.layer_num)
        values = []
        for key in coeffs_high:
            value = coeffs_high[key]
            value = torch.mul(value, value)
            values.append(value)
        output = sum(values).unsqueeze(0)

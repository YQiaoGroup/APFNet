...
import APFNet as APFNet  # nopep8
...
# 加载训练数据
train_files = APFNet.py.utils.read_file_list(args.img_list, prefix=args.img_prefix,
                                          suffix=args.img_suffix)
train_seg_files = APFNet.py.utils.read_file_list(args.train_seg_list, prefix=args.img_prefix,
                                          suffix=args.img_suffix)
...
if args.atlas:
    ...
    atlas = APFNet.py.utils.load_volfile(args.atlas, np_var='vol',
                                      add_batch_axis=True, add_feat_axis=add_feat_axis)
    seg_atlas = APFNet.py.utils.load_volfile(args.seg_atlas, np_var='vol',
                                      add_batch_axis=True, add_feat_axis=add_feat_axis)
    generator = APFNet.generators.scan_to_atlas(train_files, atlas,
                                             batch_size=args.batch_size, bidir=args.bidir,
                                             add_feat_axis=add_feat_axis)
    train_generator = APFNet.generators.seg_scan_to_atlas(train_files, atlas,
                                             batch_size=args.batch_size, bidir=args.bidir,
                                             add_feat_axis=add_feat_axis)
    train_seg_generator = APFNet.generators.seg_scan_to_atlas(train_seg_files, seg_atlas,
                                             batch_size=args.batch_size, bidir=args.bidir,
                                             add_feat_axis=add_feat_axis)
else:
    # scan-to-scan generator
    generator = APFNet.generators.scan_to_scan(
        train_files, batch_size=args.batch_size, bidir=args.bidir, add_feat_axis=add_feat_axis)
    seg_generator = APFNet.generators.semisupervised(
    train_files,train_seg_files,batch_size=args.batch_size)

# # 加载验证集的数据

val_t1_files = APFNet.py.utils.read_file_list(args.val_t1_list, prefix=args.img_prefix,
                                          suffix=args.img_suffix)
val_seg_files = APFNet.py.utils.read_file_list(args.val_seg_list, prefix=args.img_prefix,
                                          suffix=args.img_suffix)                                       
assert len(val_t1_files) > 0, 'Could not find any val data.'

atlas,atlas_affine = APFNet.py.utils.load_volfile("./AbdomenMRCT_1041_0001_image.nii.gz", np_var='vol',
                                  add_batch_axis=True, add_feat_axis=add_feat_axis,ret_affine=True)
seg_atlas = APFNet.py.utils.load_volfile(args.seg_atlas, np_var='vol',
                                      add_batch_axis=True, add_feat_axis=add_feat_axis)
val_t1_generator = APFNet.generators.seg_scan_to_atlas(val_t1_files, atlas,
                                             batch_size=args.batch_size, bidir=args.bidir,
                                             add_feat_axis=add_feat_axis)
val_seg_generator = APFNet.generators.seg_scan_to_atlas(val_seg_files, seg_atlas,
                                             batch_size=args.batch_size, bidir=args.bidir,
                                             add_feat_axis=add_feat_axis)
...
if args.image_loss == 'ncc':
    image_loss_func = APFNet.losses.NCC().loss
elif args.image_loss == 'mse':
    image_loss_func = APFNet.losses.MSE().loss
elif args.image_loss == 'MI':
    image_loss_func = APFNet.losses.MutualInformation()
elif args.image_loss == 'MIND':
    image_loss_func = APFNet.losses.MINDLoss().loss
else:
...
losses += [APFNet.losses.Grad('l2', loss_mult=args.int_downsize).loss]
weights += [args.weight]

loss_mse = [APFNet.losses.MSE().loss]
...
def train_dual_pyramid_vxm_trans_FFM_4layer_normal_GDP(config=config):
    ...
    model = APFNet.networks.dual_pyramid_VxmDense_Trans_FFM_normal_GDP()
    # 传递mednext的参数
    ...
    for epoch in range(1000):
        ...
        for step in range(100):
            ...
            # 定义空间转换层
            SPT = APFNet.torch.layers.SpatialTransformer(newshape,mode="bilinear").to(device1)
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            model.eval()
            print("---------验证集的Dice----------")
            SPT = APFNet.torch.layers.SpatialTransformer(inshape, mode="nearest").to(device1)
                ...
                for step in range(len(val_t1_files)):
                    ...
                    if step < len(val_t1_files) - 1:
                        with torch.no_grad():
                            ...
                            dice,dice_region = APFNet.torch.losses.compute_dice(seg_pre, seg_inputs_fixed[0])
                            if dice < 1:
                                print("dice", dice)
                                Dice += dice
                                # print(Dice)
...
def train_dual_pyramid_vxm_FFM_4layer_normal_GDP(config=config):
    ...
    # 准备model_layer4
    newshape = (192,160,192)    
    model = APFNet.networks.dual_pyramid_VxmDense_FFM_normal_GDP()
    # 传递mednext的参数
...
def train_dual_pyramid_vxm_FFM_4layer_large_GDP(config=config):
    ...
    # 准备model_layer4
    newshape = (192,160,192)    
    model = APFNet.networks.dual_pyramid_VxmDense_FFM_large_GDP()
    # 传递mednext的参数
    model_dict = model.state_dict()
...
def train_dual_pyramid_vxm_FFM_4layer_huge_GDP(config=config):
    ...
    model = APFNet.networks.dual_pyramid_VxmDense_FFM_huge_GDP()
    model.to(device)
    ...
    for epoch in range(1000):
        ...
        for step in range(100):
            ...
            # 对moving图像进行变换得到moved image
            # 定义空间转换层
            SPT4 = APFNet.torch.layers.SpatialTransformer((192, 160, 192), mode="bilinear").to(device1)
                ...
                for step in range(len(val_t1_files)):
                    ...
                    if step < len(val_t1_files) - 1:
                        with torch.no_grad():
                            ...
                            dice_1, _ = APFNet.torch.losses.compute_dice(seg_pre_1, seg_inputs_fixed[0])
                            dice_2, _ = APFNet.torch.losses.compute_dice(seg_pre_2, seg_inputs_fixed[0])
                            dice_3, _ = APFNet.torch.losses.compute_dice(seg_pre_3, seg_inputs_fixed[0])
...
def train_dual_pyramid_vxm_PDFNet_normal_FFM_diff_GDP(config=config):
    ...
    model = APFNet.networks.dual_pyramid_VxmDense_FFM_normal_diff_new_GDP(inshape=(192, 160, 192))
                ...
                for step in range(len(val_t1_files)):
                    ...
                    if step < len(val_t1_files) - 1:
                        with torch.no_grad():
                            ...
                            filed[:, :, :, 1] = flow[0, 1, :, :, :]
                            filed[:, :, :, 2] = flow[0, 2, :, :, :]
                            jac_det = APFNet.torch.losses.jacobian_determinant(filed.cpu())
                            Jd = np.sum(jac_det < 0)
                            if dice < 1:
                                print("dice", dice)
                                # print("dice_1", dice_1)
...
def test_dual_pyramid_vxm_FFM_adaptive_huge(config=config):
    ...
    # 读取huge model
    model_pretrain = APFNet.networks.dual_pyramid_VxmDense_FFM_huge_GDP()
    ...
    model = APFNet.networks.dual_pyramid_VxmDense_FFM_huge_adaptive_val(list_num=list_num)
    ...
    model.eval()
    print("---------验证集的Dice----------")
    SPT = APFNet.torch.layers.SpatialTransformer(inshape, mode="nearest").to(device)
    for step in range(len(val_t1_files)):
        # 提取t1数据
        inputs_fixed, y_true = next(val_t1_generator)
...
def test_dual_pyramid_vxm_FFM_adaptive_large(config=config):
    ...
    # 读取huge model
    model_pretrain = APFNet.networks.dual_pyramid_VxmDense_FFM_large_GDP()
    ...
    model = APFNet.networks.dual_pyramid_VxmDense_FFM_large_adaptive_val(list_num=list_num)
...
def test_dual_pyramid_vxm_FFM_adaptive_normal(config=config):
    ...
    # 读取huge model
    model_pretrain = APFNet.networks.dual_pyramid_VxmDense_FFM_normal_GDP()
    ...
    model = APFNet.networks.dual_pyramid_VxmDense_FFM_normal_adaptive_val(list_num=list_num)
...
def test_dual_pyramid_vxm_FFM_adaptive_normal_diff(config=config):
    ...
    # 读取huge model
    model_pretrain = APFNet.networks.dual_pyramid_VxmDense_FFM_normal_diff_new_GDP()
    ...
    model = APFNet.networks.dual_pyramid_VxmDense_FFM_normal_diff_new_adaptive_val(list_num=list_num)
...
def test_dual_pyramid_vxm_FFM_adaptive_normal_trans(config=config):
    ...
    # 读取huge model
    model_pretrain = APFNet.networks.dual_pyramid_VxmDense_Trans_FFM_normal_GDP()
    ...
    model = APFNet.networks.dual_pyramid_VxmDense_Trans_FFM_normal_adaptive_val(list_num=list_num)

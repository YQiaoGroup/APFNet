...
def pre_process(in_array):
    '''
    超分网络的预处理
    对于体素值小于0的, 置为0, 然后再将体素值放大1000倍
    '''
    in_array[in_array<0] = 0
    out_array = in_array * 1000

    return out_array


def post_process(in_array):
    '''
    超分网络的后处理
    将体素值缩小1000倍
    '''
    out_array = in_array / 1000
    out_array = torch.where(out_array>0, out_array, 0.)  # y_pred里面大于0的值才会被保留, 否则被0代替
